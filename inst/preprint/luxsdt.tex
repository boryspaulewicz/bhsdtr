% TODO Bardziej szczegółowo przedstawić wyniku testów na symulacji

% TODO Wspomnieć o gamma_to_crit

% TODO wyjaśnić sens prioru ln(100) dla gamma

% TODO opisać domyślne priory dla zeta

% mixing properties?

% constrained czy restricted?

\documentclass[oneside,a4paper]{article}
% \usepackage[MeX]{polski} %pakiety m.in. umozliwia rozne ciekawe rzeczy
% \usepackage[latin2]{inputenc}
% Bedzie osobnym punktem, na osobnej stronie
...
% The solution used in the \texttt{bhsdtr} package is based on
% the way ordered categorical variables with two or more categories,
% such as confidence ratings, are modelled in ordered logistic
% regression.
...
% Other things being equal, the greater the $\bm{\gamma}_i$ value the
% more the $\bm{c}_{j \geq i}$ criteria are shifted to the right.
...
vary between the levels of this sampled factor. % Four other optional
% elements can be used to specify the priors on random effects
% distributions. The elements named \texttt{delta\_nu} and
% \texttt{gamma\_nu} correspond to $\nu^{(\bm{\delta})}$ and
% $\nu^{(\bm{\gamma})}$ parameters and the elements named
% \texttt{delta\_scale} and \texttt{gamma\_scale} correspond to
% $\tau^{(\bm{\delta})}$ and $\tau^{(\bm{\delta})}$ parameters.
%
% Of course, random effects must be estimable within the levels of the
% sampled factor and must already be present in the fixed effects
% specification for the respective parameter, at least implicitly.
...
% KOD Specyfikacja i dopasowanie modelu
...
% The reader should be warned that ROC point posterior predictive
% intervals must be interpreted with care, because ROC points are not
% independent and it is in general not correct to expect 95\% coverage
% from 95\% ROC point posterior predictive intervals when the model is
% true.
